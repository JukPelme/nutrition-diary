"""
Unified seed script. Runs all imports in order.

Usage:
    python scripts/seed_all.py                  # offline only (быстро, ~200 продуктов)
    python scripts/seed_all.py --online         # offline + Open Food Facts API (~10K+)
    python scripts/seed_all.py --online --usda KEY  # всё включая USDA
"""
import argparse
import asyncio
import sys
...
async def main():
    ...
    parser.add_argument("--usda", type=str, default=None, help="USDA API key (получить бесплатно: fdc.nal.usda.gov)")
    ...
    if not args.online and not args.usda:
        print("\n---")
        print("Загружены только базовые продукты (офлайн).")
        print("Для расширенной базы запусти:")
        print("  python scripts/seed_all.py --online           # + Open Food Facts")
        print("  python scripts/seed_all.py --online --usda KEY # + USDA")
